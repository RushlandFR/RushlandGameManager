                ...
                new java.util.Timer().schedule(new java.util.TimerTask() {
                    ...
                    public void run() {
                        for (String players : GameData.getWaitingPlayers(port)) {
                            ...
                            RedisDataSender.publisher.publish("proxy#gamefound#" + players + "#" + CodeUtils.formatNPCType(GameManager.getInstance().getConfig().getGame()) + "#" + (gameMap.getTournamentCode() == null ? "" : "tournois-") + GameManager.getInstance().getConfig().getGame() + port + "#" + port);
                        }
                    }
                }, 500);
